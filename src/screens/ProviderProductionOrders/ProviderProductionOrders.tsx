        ...
        <div className="nb-page flex flex-col">
            <div className="flex flex-1 flex-col lg:flex-row">
                ...
                <div className="flex-1 flex flex-col min-w-0">
                    <TopNavBar>
                        <h1 className="font-extrabold text-gray-900 text-2xl">🏭 Đơn sản xuất</h1>
                        <p className="font-medium text-gray-500 text-sm mt-1">Quản lý đơn sản xuất, giao hàng và phân phối</p>
                    </TopNavBar>
                    ...
                    <main className="flex-1 px-4 sm:px-6 lg:px-10 py-6 lg:py-8 space-y-6">
                        ...
                        {loading ? (
                            ...
                            <div className="nb-card-static p-10 text-center">
                                ...
                                <p className="font-medium text-gray-400 text-base">Chưa có đơn sản xuất nào được giao.</p>
                            </div>
                        ) : (
                                    ...
                                    >
                                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                            <div className="flex-1 min-w-0">
                                                <h3 className="font-bold text-gray-900 text-base truncate">{o.batchName}</h3>
                                                <p className="font-medium text-gray-400 text-sm mt-1 truncate">
                                                    Chiến dịch: <strong className="text-gray-500">{o.campaignName}</strong> &nbsp;·&nbsp;
                                                    Trường: <strong className="text-gray-500">{o.schoolName || "—"}</strong> &nbsp;·&nbsp;
                                                    SL: <strong className="text-gray-500">{o.totalQuantity}</strong> &nbsp;·&nbsp;
                                                    {new Date(o.createdDate).toLocaleDateString("vi")}
                                                </p>
                                                {o.deliveryDeadline && (
                                                    <p className="font-medium text-gray-400 text-xs mt-1">
                                                        📅 Hạn giao: <strong className="text-gray-500">{new Date(o.deliveryDeadline).toLocaleDateString("vi")}</strong>
                                                    </p>
                                                )}
                                            </div>
                        ...
                        {showDetail && detail && (
                            <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[1000] p-4">
                                <div className="bg-white rounded-md border border-gray-200 shadow-soft-md p-8 w-full max-w-[700px] max-h-[85vh] overflow-auto">
                                    <div className="flex items-center justify-between mb-5">
                                        <h2 className="font-extrabold text-gray-900 text-xl truncate" title={detail.batchName}>📋 {detail.batchName}</h2>
                                    ...
                                    <h3 className="font-extrabold text-gray-900 text-base mb-3">📦 Danh sách sản phẩm</h3>
                                    ...
                                    {(detail.status === "Completed" || detail.status === "Delivered") && (
                                        <div className="mt-6">
                                            <div className="nb-tabs border-b-2 border-gray-200">
                                            ...
                                            {providerActiveTab === "delivery" && deliveryStatus && (
                                                <div className="py-5 space-y-4">
                                                    <div className="flex items-center justify-between">
                                                        <h3 className="font-extrabold text-gray-900 text-base">🚚 Tình trạng giao hàng</h3>
                                                    ...
                                                    <div>
                                                        <div className="flex justify-between text-sm font-medium text-gray-500 mb-2">
                                                            ...
                                                            <span className={`font-bold ${deliveryStatus.isFullyDelivered ? "text-[#10B981]" : "text-violet-600"}`}>
                                                                {Math.round((deliveryStatus.totalDelivered / deliveryStatus.totalQuantity) * 100)}%
                                                            </span>
                                                        </div>
                                                        <div className="h-3 rounded-full bg-[#E5E7EB] border border-gray-200 overflow-hidden">
                                                    ...
                                                    {deliveryStatus.deliveries.length === 0 ? (
                                                        <p className="text-center py-4 text-gray-400 font-medium">Chưa có lần giao nào.</p>
                                                    ) : (
                                                            ...
                                                            {deliveryStatus.deliveries.map((d, i) => (
                                                                <div key={d.id} className={`rounded-xl border-2 p-4 ${d.isConfirmed ? "bg-[#F0FDF4] border-[#86EFAC]" : "bg-[#FFFBEB] border-[#FDE68A]"}`}>
                                                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                                                        <div>
                                                                            <strong className="text-sm text-gray-900">Lần {deliveryStatus.deliveries.length - i}: {d.quantity} sản phẩm</strong>
                                                                            <span className="text-gray-400 text-xs ml-2">
                                                                                {new Date(d.deliveredAt).toLocaleDateString("vi")}
                                                                            </span>
                                                                            {d.note && <p className="text-gray-500 text-sm mt-1 line-clamp-3">📝 {d.note}</p>}
                                                                        </div>
                                                                        {d.isConfirmed ? (
                                                ...
                                                <div className="py-5 space-y-5">
                                                    ...
                                                    <div>
                                                        <div className="flex justify-between text-sm font-medium text-gray-500 mb-2">
                                                            ...
                                                            <span className={`font-bold ${distOverview.distributedCount === distOverview.totalOrders ? "text-[#10B981]" : "text-violet-600"}`}>
                                                    ...
                                                    {/* Schedule Timeline */}
                                                    <h4 className="font-extrabold text-gray-900 text-sm">📅 Lịch phân phối</h4>
                                                    {distOverview.schedules.length === 0 ? (
                                                        <p className="text-center py-4 text-gray-400 font-medium bg-gray-50 rounded-xl border-2 border-gray-200">
                                                                ...
                                                                <div key={s.id} className={`rounded-xl border-2 p-4 ${s.status === "Completed" ? "bg-[#F0FDF4] border-[#86EFAC]" : "bg-[#FEFCE8] border-[#FDE68A]"}`}>
                                                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                                                        <div>
                                                                            ...
                                                                            {s.note && <p className="text-gray-500 text-sm mt-1 line-clamp-3">📝 {s.note}</p>}
                                                                        </div>
                        ...
                        {showReject && detail && (
                            <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[1100] p-4">
                                <div className="bg-white rounded-md border border-gray-200 shadow-soft-md p-7 w-full max-w-[440px]">
                                    <h3 className="font-extrabold text-gray-900 text-lg mb-4">❌ Từ chối đơn sản xuất</h3>
                                    <textarea
                                        value={rejectReason}
                        ...
                        {showDeliver && detail && (
                            <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[1100] p-4">
                                <div className="bg-white rounded-md border border-gray-200 shadow-soft-md p-7 w-full max-w-[440px]">
                                    <h3 className="font-extrabold text-gray-900 text-lg mb-4">📦 Giao hàng</h3>

                                    {deliveryStatus && (
                                        <div className="mb-4">
                                            <div className="flex justify-between text-sm text-gray-500 mb-2">
                                                <span>Đã giao: {deliveryStatus.totalDelivered}/{deliveryStatus.totalQuantity}</span>
                                                <span>Còn lại: <strong className="text-gray-900">{deliveryStatus.totalQuantity - deliveryStatus.totalDelivered}</strong></span>
                                            </div>
                                            <div className="h-2 rounded-full bg-[#E5E7EB] border border-gray-200 overflow-hidden">
                                    ...
                                    <div className="space-y-3">
                                        <div>
                                            <label className="block text-sm font-bold text-gray-900 mb-1">Số lượng giao lần này</label>
                                        ...
                                        <div>
                                            <label className="block text-sm font-bold text-gray-900 mb-1">Ghi chú (tùy chọn)</label>
