  ...
  return (
    ...
    >
      <div className="mx-auto w-full max-w-[1100px] px-4 md:px-8">
        ...
        {/* 3-step cards */}
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-3 nb-reveal-stagger">
          {steps.map((step) => {
            const Icon = step.icon;
            return (
              <article
                key={step.number}
                className="nb-card-static relative bg-white px-6 pb-6 pt-12 text-center"
              >
                {/* Step number badge */}
                <div className="absolute left-1/2 top-0 flex h-12 w-12 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-xl border-3 border-[#1A1A2E] bg-[#B8A9E8] shadow-[3px_3px_0_#1A1A2E]">
                  <span className="text-lg font-extrabold text-[#1A1A2E]">{step.number}</span>
                </div>

                {/* Icon */}
                <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-xl border-3 border-[#1A1A2E] bg-[#EDE9FE] shadow-[3px_3px_0_#1A1A2E]">
                  <Icon className="h-7 w-7 text-[#1A1A2E]" strokeWidth={2.5} />
                </div>
