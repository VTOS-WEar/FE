        ...
        return (
            <GuestLayout bgColor="#f8fafc">
                <div className="mx-auto flex min-h-[70vh] max-w-[1120px] flex-col items-center justify-center px-6 py-10 text-center">
                    ...
                    <h1 className="mt-6 text-2xl font-bold text-gray-900">Không tìm thấy nhà cung cấp</h1>
                    <button onClick={() => navigate(-1)} className="mt-6 inline-flex items-center gap-2 rounded-[16px] border border-gray-200 bg-white px-5 py-3 text-sm font-bold text-gray-900 shadow-soft-sm">
    ...
    return (
        <GuestLayout bgColor="#f8fafc">
            <div className="mx-auto max-w-[1120px] px-6 py-10 lg:px-8">
                ...
                <button onClick={() => navigate(-1)} className="mt-6 inline-flex items-center gap-2 rounded-[16px] border border-gray-200 bg-white px-4 py-2.5 text-sm font-bold text-gray-900 shadow-soft-sm">
                    <ArrowLeft className="h-4 w-4" />
                    Quay lại
                </button>
                ...
                {ranking.length > 0 && (
                    <div className="mt-6 rounded-[24px] border border-gray-200 bg-white p-5 shadow-soft-sm">
                        <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-gray-400">Ranking trong trường</p>
                        <div className="mt-4 grid gap-3 md:grid-cols-3">
                            {ranking.slice(0, 3).map((provider, index) => (
                                <Link key={provider.providerId} to={`/providers/${provider.providerId}/ratings${schoolId ? `?schoolId=${schoolId}` : ""}`} className={`rounded-[18px] border p-4 transition-all hover:-translate-y-1 hover:shadow-soft-sm ${provider.providerId === ratings.providerId ? "border-violet-300 bg-violet-50" : "border-gray-200 bg-slate-50"}`}>
                                    <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-violet-500">Top {index + 1}</p>
                                    <h3 className="mt-2 text-base font-bold text-gray-900">{provider.providerName}</h3>
                ...
                <div className="mt-6 space-y-4">
                    {ratings.items.length === 0 ? (
                        <div className="rounded-[24px] border border-dashed border-gray-300 bg-white p-10 text-center shadow-soft-sm">
                            <h2 className="text-xl font-bold text-gray-900">Chưa có đánh giá nào</h2>
